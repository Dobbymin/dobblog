...
description: 참조 웹사이트를 브라우저로 열어 getComputedStyle로 색상·타이포·간격·그림자를 실측하고 재현용 디자인 스펙 문서를 만드는 절차. "디자인 조사", "색상 뽑아줘", "원본 실측", "토큰 추출", "이 사이트처럼", "클론할 스펙 정리", "구름 어떻게 만들어졌는지" 같은 요청이면 반드시 이 스킬을 사용하라. 구름/파형 배경을 CSS로 재현하는 레시피도 포함한다.
---
